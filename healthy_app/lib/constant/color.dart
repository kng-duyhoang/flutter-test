import 'package:flutter/material.dart';

class AppColor {
  // Light Color
  // Primary Color
  static const iconLightColor = Color(0xFF171725);
  static const lightPrimaryColor = Color(0xFF1F4690);

  // Secondary Color
  static const lightSecondColor = Color(0xFFFE970F);
  // Dark Color
  static const darkPrimaryColor = Color(0xFF171725);
  static const iconDarkColor = Color(0xFFFE970F);

  // Other Color
  static const white1 = Color.fromARGB(255, 255, 255, 255);
  static const lightColor1 = Color(0xFFCED1D9);
  static const lightColor2 = Color(0xFF828899);
  static const lightColor3 = Color(0xFFBACAFF);
  static const lightColor4 = Color(0xFF3F5AA6);
  static const defaultColor = Color(0xFF111111);
  static const borderColor = Color(0xFFE3E7EC);
  static const grey = Color(0xFF6C6C6C);
  
}
